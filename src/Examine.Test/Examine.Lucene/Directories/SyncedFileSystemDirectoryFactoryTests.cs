...
namespace Examine.Test.Examine.Lucene.Directories
{
    ...
    public class SyncedFileSystemDirectoryFactoryTests : ExamineBaseTest
    {
        ...
        [Test]
        public void Repro_Restart_WithTaxonomy_When_ScheduledReplication_Then_NewItemsReplicate()
        {
            WithTempPaths((mainPath, tempPath) =>
            {
                var facetFields = new FieldDefinitionCollection(
                    new FieldDefinition("cat", FieldDefinitionTypes.FacetTaxonomyFullText));

                LuceneIndex MakeIndex(out SyncedFileSystemDirectoryFactory f)
                {
                    var dirOptions = new LuceneDirectoryIndexOptions { UseTaxonomyIndex = true };
                    f = new SyncedFileSystemDirectoryFactory(
                        new DirectoryInfo(tempPath),
                        new DirectoryInfo(mainPath),
                        new DefaultLockFactory(),
                        LoggerFactory,
                        CreateDirectoryOptionsMonitor(dirOptions),
                        false);
                    var options = new LuceneDirectoryIndexOptions
                    {
                        DirectoryFactory = f,
                        UseTaxonomyIndex = true,
                        FieldDefinitions = facetFields,
                    };
                    return new LuceneIndex(LoggerFactory, TestIndex.TestIndexName, CreateDirectoryOptionsMonitor(options));
                }

                ValueSet FacetValueSet(string id, string cat)
                    => new ValueSet(id, "content",
                        new Dictionary<string, IEnumerable<object>>
                        {
                            { "item1", new List<object>(new[] { "value1" }) },
                            { "cat", new List<object>(new[] { cat }) }
                        });

                // RUN 1: fresh build, index items (with taxonomy facets), replicate to main, then "stop".
                using (var index1 = MakeIndex(out _))
                {
                    using (index1.WithThreadingMode(IndexThreadingMode.Synchronous))
                    {
                        for (var i = 0; i < 10; i++)
                        {
                            index1.IndexItem(FacetValueSet(i.ToString(), "cat" + (i % 3)));
                        }
                    }

                    var mainIndexPath1 = Path.Combine(mainPath, TestIndex.TestIndexName);
                    Assert.IsTrue(WaitForCondition(() => MainIndexDocCount(mainIndexPath1) == 10), "Run1 replication failed");
                }

                // Clear local temp to mimic the customer deleting AppData temp folder before restart.
                DeleteDirectoryIfExists(tempPath);

                // RUN 2: restart - main index + taxonomy already exist. Issue a DELETE first
                // (a delete-only commit never initializes the taxonomy writer), mirroring Umbraco's
                // reindex handler which can process deletes before adds.
                var errors = new List<string>();
                using (var index2 = MakeIndex(out _))
                {
                    index2.IndexingError += (s, e) => errors.Add(e.Message + " :: " + e.Exception);

                    using (index2.WithThreadingMode(IndexThreadingMode.Synchronous))
                    {
                        // delete-only first commit
                        index2.DeleteFromIndex(new[] { "0" });
                    }

                    var mainIndexPath2 = Path.Combine(mainPath, TestIndex.TestIndexName);
                    var replicated = WaitForCondition(() => MainIndexDocCount(mainIndexPath2) == 9);
                    TestContext.WriteLine("ERRORS: " + string.Join("\n", errors));
                    Assert.IsTrue(replicated, "Run2 (restart) delete was NOT replicated to main. Errors: " + string.Join("\n", errors));
                }
            });
        }
