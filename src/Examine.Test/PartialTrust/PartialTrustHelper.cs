			...
			public void Run(MethodInfo methodToRun)
			{
				...
				catch (TargetInvocationException ex)
				{
					if (ex.InnerException != null && (ex.InnerException is SecurityException || ex.InnerException is MemberAccessException))
					{
						...
						throw new PartialTrustTestException(string.Format(CultureInfo.InvariantCulture, "Test {0} fails in a partial trust environment, due to: {1}", methodToRun.Name, inner.Message), ex.InnerException);
					}
					throw;
				}
			}
